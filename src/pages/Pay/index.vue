...
<script>
import { reqgetOder ,reqcheckPayState} from "@/api";
...
export default {
  ...
  mounted() {
    reqgetOder(this.$route.query.orderId).then(
      (value) => {
        if (value.code == 200) {
      ...
      QRCode.toDataURL(this.orderInfo.codeUrl)
        ...
        if(!this.timer){
          this.timer =setInterval(async() => {
           let ruselt = await reqcheckPayState(this.$route.query.orderId)
           if(ruselt.code ==205 || ruselt.code ==200 ){
             clearInterval(this.timer)
             this.timer = null 
             this.code = ruselt.code
             this.$msgbox.close()
             this.$router.push('/paySuccess')
           }
          }, 2000);
